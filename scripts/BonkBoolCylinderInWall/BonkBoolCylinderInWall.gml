...
function BonkBoolCylinderInWall(_cylinder, _wall)
{
    with(_cylinder)
    {
        if ((z - 0.5*height > _wall.z2) || (z + 0.5*height <= _wall.z1)) return false;
        
        return __BonkBoolCylinderInWall(x, y, radius, _wall.x1, _wall.y1, _wall.x2, _wall.y2);
    }
    
    return false;
}

function __BonkBoolCylinderInWall(_cx, _cy, _r, _x1, _y1, _x2, _y2)
{

...
function __BonkClassMeshWorker(_mesh, _vertexBufferArray, _vertexFormat, _matrix, _applySoftEdges) constructor
{
    static _pendingWorkerArray = __BonkSystem().__pendingWorkerArray;
    
    if (BONK_DEBUG_VERTEX_BUFFER_ASYNC)
    {
        __BonkTrace($"Adding worker {string(ptr(self))} to world {string(ptr(_mesh))}");
    }
    ...
    array_push(__mesh.__bonkWorkerArray, self);
    ...
    static __End = function()
    {
        ...
        if (BONK_DEBUG_VERTEX_BUFFER_ASYNC)
        {
            __BonkTrace($"Removing worker {string(ptr(self))} from world {string(ptr(__mesh))}");
        }
        ...
        with(_mesh)
        {
            var _index = array_get_index(__bonkWorkerArray, other);
            if (_index >= 0)
            {
                array_delete(__bonkWorkerArray, _index, 1);
            }
            else
            {
                __BonkTrace($"Warning! Could not find worker {string(ptr(self))} in mesh {string(ptr(self))}");
                break;
            }
    ...
    static __WorkTriangles = function()
    {
        ...
        repeat(min(BONK_VERTEX_BUFFER_ASYNC_TRIANGLE_RESOLUTION, __trianglesRemaining))
        {
            ...
            _workMinCellZ = min(_workMinCellZ, _cellZMin, _cellZMax);
            
            _workMaxCellX = max(_workMaxCellX, _cellXMin, _cellXMax);
            _workMaxCellY = max(_workMaxCellY, _cellYMin, _cellYMax);
            _workMaxCellZ = max(_workMaxCellZ, _cellZMin, _cellZMax);
